...
const Modal: React.FC<ModalProps> = ({ 
  ...
  return (
    <div 
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-3 md:p-4 animate-in fade-in"
      onClick={handleOverlayClick}
    >
      <div 
        className={cn(
          "relative bg-background rounded-lg shadow-lg flex flex-col max-h-[calc(100vh-2rem)] md:max-h-[90vh] w-full animate-in slide-in-from-bottom-4",
          sizeStyles[size],
          className
        )}
        role="dialog"
      ...
      >
        {(title || onClose) && (
          <div className="flex items-center justify-between p-3 md:p-4 border-b flex-shrink-0">
            {title && (
              <h2 id="modal-title" className="text-sm md:text-base font-semibold pr-2">{title}</h2>
            )}
            {onClose && (
              <button
