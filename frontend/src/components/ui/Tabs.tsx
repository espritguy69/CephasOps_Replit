...
const Tabs: React.FC<TabsProps> = ({ children, defaultActiveTab, onTabChange, className = '' }) => {
  ...
  return (
    <div className={cn("w-full", className)}>
      <div className="flex flex-wrap gap-1 md:inline-flex h-8 md:items-center md:justify-center rounded bg-muted p-0.5 text-muted-foreground" role="tablist">
        {tabs.map((tab) => (
          <button
            key={tab.index}
            className={cn(
...
export const TabPanel: React.FC<TabPanelProps> = ({ children, label, icon, disabled, active, className = '' }) => {
