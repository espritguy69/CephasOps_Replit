...
import { returnStock } from '../../api/ledger';
...
import type { LedgerReturnRequest } from '../../types/ledger';
...
const InventoryReturnPage: React.FC = () => {
    ...
    try {
      if (serialisedMode && serialLines.trim()) {
        ...
        for (const serial of serials) {
          const body: LedgerReturnRequest = {
            orderId,
            materialId,
            locationId,
            quantity: 1,
          ...
          await returnStock(body, departmentId);
        }
        showSuccess(`Returned ${serials.length} serialised item(s).`);
      } else {
        const body: LedgerReturnRequest = {
          orderId,
          materialId,
        ...
        await returnStock(body, departmentId);
        showSuccess('Stock returned successfully.');
      }
      ...
      if (apiErr?.status === 403) {
        ...
        const msg = (err as Error).message || 'Failed to return stock';
        setError(msg);
        showError(msg);
      }
    } finally {
  ...
  if (!departmentId) {
    return (
      <PageShell title="Return Stock" breadcrumbs={[{ label: 'Inventory', path: '/inventory/stock-summary' }, { label: 'Return' }]}>
        <EmptyState title="Department required" description="Please select a department from the header to return stock." />
      </PageShell>
    );
  }
  ...
  return (
    <PageShell title="Return Stock" breadcrumbs={[{ label: 'Inventory', path: '/inventory/stock-summary' }, { label: 'Return' }]}>
      <div className="space-y-4">
        <p className="text-sm text-muted-foreground">Return stock from an order to a location.</p>
...
export default InventoryReturnPage;
