...
import { allocateStock } from '../../api/ledger';
...
import type { LedgerAllocateRequest } from '../../types/ledger';
...
const InventoryAllocatePage: React.FC = () => {
    ...
    try {
      if (serialisedMode && serialLines.trim()) {
        ...
        for (const serial of serials) {
          const body: LedgerAllocateRequest = {
            orderId,
            materialId,
            locationId,
            quantity: 1,
          ...
          await allocateStock(body, departmentId);
        }
        showSuccess(`Allocated ${serials.length} serialised item(s) to order.`);
      } else {
        const body: LedgerAllocateRequest = {
          orderId,
          materialId,
      ...
      if (apiErr?.status === 403) {
        ...
        const msg = (err as Error).message || 'Failed to allocate stock';
        setError(msg);
        showError(msg);
      }
    } finally {
  ...
  if (!departmentId) {
    return (
      <PageShell title="Allocate Stock to Order" breadcrumbs={[{ label: 'Inventory', path: '/inventory/stock-summary' }, { label: 'Allocate' }]}>
        <EmptyState title="Department required" description="Please select a department from the header to allocate stock." />
      </PageShell>
    );
  }
  ...
  return (
    <PageShell title="Allocate Stock to Order" breadcrumbs={[{ label: 'Inventory', path: '/inventory/stock-summary' }, { label: 'Allocate' }]}>
      <div className="space-y-4">
        <p className="text-sm text-muted-foreground">Reserve stock for an order. Allocation prevents double-use of the same stock.</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-2 pt-2">
            <Button type="submit" disabled={submitting || !orderId || !materialId || !locationId || (serialisedMode ? !serialLines.trim() : false)}>
              {submitting ? 'Submitting…' : 'Allocate'}
            </Button>
...
export default InventoryAllocatePage;
