...
import { transferStock } from '../../api/ledger';
import { getMaterials, getStockLocations } from '../../api/inventory';
import { Card, Button, Input, Label, useToast, EmptyState } from '../../components/ui';
import { PageShell } from '../../components/layout';
import type { LedgerTransferRequest } from '../../types/ledger';
...
const InventoryTransferPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const body: LedgerTransferRequest = {
        materialId,
        fromLocationId,
        toLocationId,
        quantity,
      ...
      await transferStock(body, departmentId);
      ...
      if (apiErr?.status === 403) {
        ...
        const msg = (err as Error).message || 'Failed to transfer stock';
        setError(msg);
        showError(msg);
      }
    } finally {
  ...
  if (!departmentId) {
    return (
      <PageShell title="Transfer Stock" breadcrumbs={[{ label: 'Inventory', path: '/inventory/stock-summary' }, { label: 'Transfer' }]}>
        <EmptyState title="Department required" description="Please select a department from the header to transfer stock." />
      </PageShell>
    );
  }
      ...
      <div className="space-y-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-2 pt-2">
            <Button type="submit" disabled={submitting || !materialId || !fromLocationId || !toLocationId}>
