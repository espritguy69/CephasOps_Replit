...
- UI/UX gaps compared to `/new_docs` specifications
...
**Required Work:**
- 🔴 Create full React + Vite project structure
...
## 2. Admin Frontend Page Inventory vs. `/new_docs`

### 2.1 Pages That Exist ✅

Based on `/new_docs/CephasOps_Frontend_Page_Inventory.md`:
...
- ✅ SI-specific API functions (`src/api/si-app.ts`)
...
### 4.2 Required SI App Pages (from `/new_docs/CephasOps_SI_App_Overview.md`)

| Page | Route | Status | Priority |
|------|-------|--------|----------|
| Job List | `/jobs` | ❌ Missing | 🔴 Critical |
| Job Detail | `/jobs/:orderId` | ❌ Missing | 🔴 Critical |
| Status Transition | Part of Job Detail | ❌ Missing | 🔴 Critical |
| Checklist Completion | Part of Job Detail | ❌ Missing | 🔴 Critical |
| Material Usage | Part of Job Detail | ❌ Missing | 🟡 High |
| Photo Upload | Part of Job Detail | ❌ Missing | 🟡 High |
| Serial Number Scan | Part of Job Detail | ❌ Missing | 🟡 High |
| GPS Tracking | Automatic | ❌ Missing | 🟡 High |
...
**Missing Features:**
- ⚠️ **Sub-steps rendering** - Backend supports it, but UI may not display hierarchical structure
- ⚠️ **Sub-steps validation** - Need to verify UI enforces sub-step completion rules
- ⚠️ **Visual indentation** - Sub-steps should be visually indented in UI
...
- SI app requires mobile-first approach (not yet implemented)
...
1. **Complete SI App Foundation** (Week 1)
   - Set up project structure
   - Install Tailwind v4.0
   - Create base layout

2. **Implement SI Core Pages** (Week 2)
   - Job List
   - Job Detail
   - Status transitions

3. **Enhance Checklist UI** (Week 2)
   - Sub-steps rendering
   - Hierarchical display

4. **Add SI Features** (Week 3)
   - Photo upload
   - GPS tracking
   - Serial scanning

5. **Fill Admin Gaps** (Week 3-4)
   - Missing pages
   - Rate Engine UI
   - KPI module
...
**Next Review:** After SI App Foundation Complete
