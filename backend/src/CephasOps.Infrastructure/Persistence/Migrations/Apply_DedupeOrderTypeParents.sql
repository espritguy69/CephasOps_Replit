-- Migration: 20260308150000_DedupeOrderTypeParentsAndAddUniqueIndexes
-- Run this script to apply the Order Type dedupe and unique indexes, then record in history.

-- 1) Cleanup duplicate parents
...
-- 3) Create unique partial indexes
CREATE UNIQUE INDEX IF NOT EXISTS "IX_OrderTypes_CompanyId_Code_Parents"
ON "OrderTypes" ("CompanyId", "Code")
WHERE "ParentOrderTypeId" IS NULL AND ("IsDeleted" IS NOT DISTINCT FROM false);

-- 3b) Deduplicate subtypes so the next index can be created (keep one per CompanyId, ParentOrderTypeId, Code)
...
VALUES ('20260308150000_DedupeOrderTypeParentsAndAddUniqueIndexes', '10.0.3')
