...
## Tenant-scoped and tenant-safety tests

Tests that add or modify **tenant-scoped entities** (e.g. `CompanyScopedEntity`, `User`, `BackgroundJob`, `Notification`) must set **`TenantScope.CurrentTenantId`** (or use `TenantSafetyGuard.EnterPlatformBypass()` for platform-wide setup) before calling `SaveChangesAsync`, and restore in `finally`. Test classes that manipulate `TenantScope` should use `[Collection("TenantScopeTests")]` to avoid parallel execution overwriting scope. For the **tenant-safety regression suite** (exact command in Section 7.1), isolated tests, and in-memory/global-filter limitations, see [TENANT_SAFETY_DEVELOPER_GUIDE.md](../architecture/TENANT_SAFETY_DEVELOPER_GUIDE.md) Section 7 (Section 7.1 has the exact `dotnet test` command and filter for the stable regression suite).
