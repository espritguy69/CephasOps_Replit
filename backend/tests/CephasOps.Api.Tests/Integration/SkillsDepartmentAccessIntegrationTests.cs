...
/// <summary>
/// Integration test proving department-scoped skills access:
/// a user in department A cannot access department B's skills (403).
/// </summary>
[Collection("InventoryIntegration")]
public class SkillsDepartmentAccessIntegrationTests : IClassFixture<CephasOpsWebApplicationFactory>
{
    private readonly CephasOpsWebApplicationFactory _factory;

    public SkillsDepartmentAccessIntegrationTests(CephasOpsWebApplicationFactory factory)
    {
        _factory = factory;
    }

    [Fact]
    public async Task GetSkillsByCategory_UserInDeptA_RequestingDeptB_Returns403()
    {
        ...
        var response = await client.GetAsync(
            $"api/skills/by-category?departmentId={deptBId}");
