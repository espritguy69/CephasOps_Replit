...
**Enforcement:** Order transitions are enforced by **SiWorkflowGuard** in application code (see [SI_APP_WORKFLOW_HARDENING_REPORT.md](../../backend/docs/operations/SI_APP_WORKFLOW_HARDENING_REPORT.md)). Invalid jumps are rejected; transition to ReschedulePendingApproval requires a non-empty Reason; completion (OrderCompleted/Completed) cannot skip the prior MetCustomer milestone.
